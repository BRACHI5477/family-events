...
router.get('/summary', (req, res) => {
  const membersTotal = db.prepare('SELECT COUNT(*) c FROM FamilyMembers WHERE archived = 0').get().c;
  const membersArchived = db.prepare('SELECT COUNT(*) c FROM FamilyMembers WHERE archived = 1').get().c;
  const eventsTotal = db.prepare('SELECT COUNT(*) c FROM Events WHERE active = 1').get().c;

  const byType = db.prepare(`
    SELECT et.name, et.icon, et.color, COUNT(e.id) c
    FROM EventTypes et LEFT JOIN Events e ON e.type_id = et.id AND e.active = 1
    GROUP BY et.id ORDER BY c DESC`).all();
  ...
  const upcoming = occurrencesInRange(fmtGreg(today), fmtGreg(in30));
...
router.get('/by-month', (req, res) => {
  ...
  const list = occurrencesInRange(`${year}-01-01`, `${year}-12-31`);
